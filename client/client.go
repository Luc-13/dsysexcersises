...
var server gRPC.TempClient      //the server
...
func ConnectToServer() {
	...
	server = gRPC.NewTempClient(conn)
	ServerConn = conn
...
func incrementVal(val string) {
	...
	ack, err := server.Increase(context.Background(), amount)
	if err != nil {
...
func sayHi() {
	...
	stream, err := server.Greet(context.Background())
	if err != nil {
		log.Println(err)
		return
	}
	...
	stream.Send(&gRPC.Msg{Name: *clientsName, Msg: "Hi"})
	stream.Send(&gRPC.Msg{Name: *clientsName, Msg: "How are you?"})
	stream.Send(&gRPC.Msg{Name: *clientsName, Msg: "I'm fine, thanks."})
...
func conReady(s gRPC.TempClient) bool {
